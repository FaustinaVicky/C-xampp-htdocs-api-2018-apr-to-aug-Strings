...
function clean($string) {
   ...
   return preg_replace('/[^a-zA-Z]/', '',ucwords(strtolower( $string))); 
}
...
function remArrow($string) {
   $arrow = array('^','-', '@', '/', ' ');
   return str_replace($arrow, null, $string);
}

 $wce_array =
 explode(',',$wambua_corrupted_sample);
 $column_number = 8;
?>
<table border="2" cellpadding='6' cellspacing= '0'>
  <thead>
    <?php $row_num = count($wce_array) / 8; $counter = 0;?>
    <?php for($i = 0;$i<$row_num;$i++): ?>
        <tr>
          <?php for($j =0; $j<8; $j++): ?>
            <th><?php echo clean(remArrow(ucwords(strtolower(lcfirst($wce_array[$counter]))))); ?></th>
          <?php $counter++; endfor; ?>
        </tr>
      <?php endfor; ?>
    </thead>
